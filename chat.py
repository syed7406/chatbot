...
def send_message():
    if request.method == 'POST':
        ...
        for question in question_answers.keys():
            score = fuzz.ratio(user_input, question.lower())
            if score > best_score and score > 70:  # 70% similarity threshold
                best_score = score
                best_match = question_answers[question]
        
        return jsonify({'response': best_match if best_match else "I only answer predefined questions."})
